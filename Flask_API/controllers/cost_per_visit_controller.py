...
def city():
    ...
    sql = pgsql.SQL(get_kpi_query("cost_per_visit")['M']).format(pgsql.Identifier(DB_TABLE))    # params = [city, limit]
    return query_execution(DB_TABLE,limit,sql)
         

@cost_per_visit.route("/cost/visit/weekly", methods=["GET"])
# @jwt_required
def weekly():
    """ Query weather data for a specific city with an optional limit.
        http://localhost:5000/db_warranty_claims/cost/visit/weekly?limit=1000
    """
    # print("DB_TABLE:", DB_TABLE)
    # print("DB_QUERY_LIMIT:", DB_QUERY_LIMIT)
    # city = request.args.get("city")
    limit = request.args.get("limit", type=int) or DB_QUERY_LIMIT   
    # sql = pgsql.SQL("SELECT * FROM {} WHERE city = %s LIMIT %s").format(pgsql.Identifier(table))
    sql = pgsql.SQL(get_kpi_query("cost_per_visit")['W']).format(pgsql.Identifier(DB_TABLE))    # params = [city, limit]
    return query_execution(DB_TABLE,limit,sql)
