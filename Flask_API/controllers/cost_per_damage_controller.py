...
# uitils\kpi_query_utils.py
from uitils.kpi_query_utils import get_kpi_query

DB_TABLE = DBQueryUtils.DB_TABLE 
DB_QUERY_LIMIT = DBQueryUtils.DB_QUERY_LIMIT 

cost_per_damage = Blueprint("cost_per_damage", __name__, url_prefix="/kpi")

@cost_per_damage.route("/cost/damage", methods=["GET"])
# @jwt_required
def city():
    ...
    sql = pgsql.SQL(get_kpi_query("cost_per_damage")).format(pgsql.Identifier(DB_TABLE))    # params = [city, limit]
