...
hours_per_vin = Blueprint("hours_per_vin", __name__, url_prefix="/kpi")

@hours_per_vin.route("/hours/vin", methods=["GET"])
# @jwt_required
def city():
    ...
    sql = pgsql.SQL(get_kpi_query("hours_per_vin")['M']).format(pgsql.Identifier(DB_TABLE))    # params = [city, limit]
    return query_execution(DB_TABLE,limit,sql)
         
@hours_per_vin.route("/hours/vin/weekly", methods=["GET"])
# @jwt_required
def weekly():
    """ Query weather data for a specific city with an optional limit.
        http://localhost:5000/db_warranty_claims/cost/vist?limit=1000
    ...
    limit = request.args.get("limit", type=int) or DB_QUERY_LIMIT   
    # sql = pgsql.SQL("SELECT * FROM {} WHERE city = %s LIMIT %s").format(pgsql.Identifier(table))
    sql = pgsql.SQL(get_kpi_query("hours_per_vin")['W']).format(pgsql.Identifier(DB_TABLE))    # params = [city, limit]
    return query_execution(DB_TABLE,limit,sql)
