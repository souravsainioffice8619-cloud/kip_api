...
worst_performance_router = APIRouter(prefix="/kpi")


@worst_performance_router.get("/worst_performance")
def get_worst_performance(
    request: Request,
    limit: int | None = Query(default=5),
    _: dict = Depends(jwt_required),
):
    """Retrieve dealers with the worst performance based on cost per VIN."""
