...
lowest_warranty_issues_by_models_router = APIRouter(prefix="/kpi")


@lowest_warranty_issues_by_models_router.get("/lowest_warranty_issues_by_models")
def get_lowest_warranty_issues_by_models(
    request: Request,
    limit: int | None = Query(default=5),
    _: dict = Depends(jwt_required),
):
    """Retrieve models with lowest warranty claim counts."""
