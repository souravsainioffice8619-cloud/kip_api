...
cost_per_visit_router = APIRouter(prefix="/kpi")


@cost_per_visit_router.get("/cost/visit")
...
@cost_per_visit_router.get("/cost/visit/weekly")
def weekly(request: Request, limit: int | None = Query(default=None)):
    ...
    sql = pgsql.SQL(
        ...
        SELECT
            ...
            ROUND(
                SUM(CAST(REPLACE(total_cost, ',', '') AS DECIMAL))
                / NULLIF(COUNT(fin || repair_date), 0),
                2
            ) AS cost_per_visit
        FROM {}
